...
# ROS lib
import rospy
from sensor_msgs.msg import Imu, Image#, TimeReference
...
# Creates a ROS node with subscribers
class data_exchange_node():
    def __init__(self):
        rospy.init_node('cat_maus_app',anonymous=True)
        self.img_subscriber = rospy.Subscriber('/us_image', Image, self.update_img)
        self.imu_subscriber = rospy.Subscriber('/us_imu', Imu, self.update_imu)
        self.img = Image()
        self.imu = Imu()
    def update_img(self,data):
        self.img = data
        Data.img_buff = np.array(bytearray(self.img.data)).reshape(self.img.height,self.img.width,4)
        #Data.add([timestamp, Data.img_buff, micronsPerPixel]) # Data is a class for data exchangeing
...
class CatMausApp: 
    ...
    def Connect1(self):
        if not self.var_con1:
            self.data_node=data_exchange_node()
            self.var_con1 = True
    ...
    def replay_img(self,n):
        ...
        if len(self.data_out) != 0:
            ...
            # self.Seg1.set_xdata(segx)
            # self.Seg1.set_ydata(segy)
        # if len(self.data_out2) != 0:
        #     img2 = self.out2[n][3]
        #     segx = self.out2[n][1]
        #     segy = self.out2[n][2]
        #     self.USI2.set_data(img2)
        #     self.Seg2.set_xdata(segx)
        #     self.Seg2.set_ydata(segy)
        self.canvas.draw()
    def R_forward(self):
        n = self.var_Rind.get()
    ...
    def saveimgdata(self):
        ...
        #Save_as_mat(self.data_out,initfilename)
        # prepare new data list to save
        # if self.save_1Var.get() or self.save_2Var.get():
        #     initfilename = self.file_nameVar.get()
        #     if self.save_1Var.get(): Save_as_dat(self.out1,initfilename + "_P1")
        #     if self.save_2Var.get(): Save_as_dat(self.out2,initfilename + "_P2")
        # else:
        #     msg_window(self.master,'Select at least one probe to save!')
        # return
    def saveimudata(self):
        ...
        img_tm= imu_array[:,14]
        file_name = tk.filedialog.asksaveasfilename(initialdir='/Users/catandmaus/Dropbox/OxfordPostdoc/CATMAUS/', initialfile= initfilename,filetypes=[('Data files', '*.mat')],defaultextension='.mat')
        if file_name:
            savemat(file_name,{
                "time":tm,
                "acc":acc,
        ...
        # Save_as_dat(self.imu_out,initfilename+"_imu")
